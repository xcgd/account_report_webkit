...
class CommonBalanceReportHeaderWebkit(CommonReportHeaderWebkit):
    ...
    def compute_balance_data(self, data, filter_report_type=None):
        ...
        for account in self.pool.get('account.account').browse(self.cursor, self.uid, account_ids):
            ...
            # if any amount is != 0 in comparisons, we have to display the whole account
            display_account = include_zero
            comp_accounts = []
            ...
            if analytic_codes:
                ...
                if a_code:
                    if a_code.id not in analytic_groups:
                        ...
                        a_row.credit += account.credit
                        a_row.balance += account.balance
